...
def load_level_config(level):
  ...
  level_config_path = f'./resources/levels/{level}/config.json'
...
def load_level_resources(level_config):
  ...
  background_path = os.path.join(level_config["base_path"],
                                 level_config["background"])
  thumb_path = os.path.join(level_config["base_path"], level_config["thumb"])
  collisions_path = os.path.join(level_config["base_path"],
                                 level_config["collisions"])
...
def point_inside_rect(point, rect):
  ...
  print(rect.x <= x <= rect.x + rect.width
...
background, collision_map, screen_size, map_thumb = load_level_resources(
  level_config)
...
player_location = (level_config["startx"] * TILESIZE,
                   level_config["starty"] * TILESIZE)
...
# Set up the chat box
chat_box_height = 100
chat_box_rect = pygame.Rect(0,
                            screen.get_height() - chat_box_height,
                            screen.get_width(), chat_box_height)
chat_box_surface = pygame.Surface(chat_box_rect.size)
chat_box_surface.fill((20, 20, 20))

# Set up the text input
font = pygame.font.Font(None, 36)
input_box_rect = pygame.Rect(10, chat_box_rect.height - 40,
                             chat_box_rect.width - 120, 30)
...
send_button_rect = pygame.Rect(input_box_rect.right + 10, input_box_rect.y,
                               100, 30)
...
send_button_text_surface = send_button_font.render(send_button_text, True,
                                                   send_button_color)
...
while running:
  ...
  for tloc in level_config["trigger_locations"]:
    ...
    if (player.rect.centerx // TILESIZE == tloc['trigger_xy'][0]
  ...
  input_box_background_color = ((255, 255, 255) if input_box_active else
                                (200, 200, 200))
  pygame.draw.rect(chat_box_surface, input_box_background_color,
                   input_box_rect)
  pygame.draw.rect(chat_box_surface, input_box_color, input_box_rect, 2)
  chat_box_surface.blit(text_surface,
  ...
  chat_box_surface.blit(send_button_text_surface,
                        (send_button_rect.x + 30, send_button_rect.y + 3))
